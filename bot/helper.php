...
function check_profile($client, $userId){
	$db = new Database();
	$r = $db->query("SELECT * FROM customers WHERE user_id = '{$userId}'");
	if($r->num_rows){
		$result = $r->fetch_object();
		if(!$result->following){
			$db->query('UPDATE customers SET `following` = 1 WHERE id = ' . $result->id);
		}
	}else{
		$profile = $client->getProfil($userId);
		if($profile){
			$db->query("INSERT INTO customers (user_id, display_name, picture_url) 
